...
export const NotificationsPage = () => {
  ...
  return (
    <section className="content-page">
      ...
      {query.isError ? <StatePanel kind="error" title="Không tải được thông báo" description="Vui lòng thử lại sau." /> : null}
      {query.data?.items.length === 0 ? (
        <StatePanel kind="empty" title="Chưa có thông báo" description="Các cập nhật học tập sẽ được ghi lại ở đây." />
      ) : (
          ...
          {query.data?.items.map((item) => (
            <article key={item.id} className={item.readAt ? "" : "unread"}>
              <span>
                ...
                <small>{formatDateTime(item.createdAt)} · {item.eventType}</small>
              </span>
              <p>{item.body}</p>
              <Badge tone={item.readAt ? "neutral" : "info"}>{item.readAt ? "Đã đọc" : "Mới"}</Badge>
            </article>
          ))}
        </div>
      )}
