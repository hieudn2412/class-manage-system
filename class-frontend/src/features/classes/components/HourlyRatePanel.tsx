...
export const HourlyRatePanel = ({ classId }: { classId: string }) => {
  ...
  return <section className="panel-flat section-panel hourly-rate-panel"><header><div><p className="eyebrow">RATE HISTORY / FL-12</p><h2 className="section-title">Lịch sử đơn giá dạy</h2></div>{canManage ? <Button variant="secondary" onClick={() => open("new")}><CirclePlus size={16} /> Thêm mức giá</Button> : <History size={20} />}</header>
    {query.isError ? <StatePanel kind="error" title="Không tải được đơn giá" description="Vui lòng thử lại." actionLabel="Thử lại" onAction={() => void query.refetch()} /> : <div className="rate-timeline">{query.data?.map((rate, index) => <article key={rate.id} className={index === 0 ? "current" : ""}><span className="rate-dot" /><div><strong>{formatCurrency(rate.hourlyRate)}/giờ</strong><small>{formatDate(rate.effectiveDate)} → {rate.effectiveTo ? formatDate(rate.effectiveTo) : "hiện tại"}</small></div><span>v{rate.version}</span>{canManage ? <Button variant="ghost" onClick={() => open(rate)}>Sửa</Button> : null}</article>)}</div>}
    <Modal open={Boolean(editing)} title={editing === "new" ? "Thêm đơn giá hiệu lực" : "Sửa đơn giá"} onClose={() => setEditing(null)} confirmLabel="Lưu và tính lại" confirmDisabled={!effectiveDate || Number(hourlyRate) <= 0 || !reason.trim()} confirmLoading={mutation.isPending} onConfirm={() => mutation.mutate()}><div className="modal-form"><Input label="Ngày bắt đầu hiệu lực" type="date" value={effectiveDate} onChange={(event) => setEffectiveDate(event.target.value)} /><Input label="Đơn giá/giờ (VND)" type="number" min="1" value={hourlyRate} onChange={(event) => setHourlyRate(event.target.value)} /><Textarea label="Lý do thay đổi" value={reason} onChange={(event) => setReason(event.target.value)} hint="Các buổi Completed bị ảnh hưởng sẽ tự tăng salary revision." /></div></Modal>
  </section>;
};
