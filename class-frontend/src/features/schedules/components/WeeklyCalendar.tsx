...
const mobileStateLabels: Record<CalendarSession["scheduleState"], string> = {
  UPCOMING: "Sắp tới",
  TAUGHT: "Đã dạy",
  MISSING_CHECK_IN: "Chưa xác nhận",
  CANCELLED: "Đã hủy",
};
...
export const WeeklyCalendar = ({ schedule, onSelectSessions }: WeeklyCalendarProps) => {
  ...
  return (
    <section className="calendar-board" aria-label="Thời khóa biểu tuần">
      <p className="calendar-mobile-hint" id="calendar-mobile-hint">
        Vuốt ngang để xem đủ các ngày trong tuần
      </p>
      <div
        className="calendar-scroll calendar-week-view"
        tabIndex={0}
        aria-label="Thời khóa biểu tuần, có thể cuộn ngang"
        aria-describedby="calendar-mobile-hint"
      >
                    ...
                    <div className="calendar-cell" role="gridcell" key={`${date}-${time}`}>
                      {group && session ? (
                        <button
                          ...
                          className={cn(
                            "calendar-event",
                            group.sessions.length > 1 && "is-concurrent",
                            session.status === "CANCELLED" && "is-cancelled",
                          )}
                          data-state={session.scheduleState}
                        ...
                        >
                          {group.sessions.length > 1 ? (
                            ...
                            <>
                              ...
                              <span className="calendar-event-mobile-state" aria-hidden="true">
                                {mobileStateLabels[session.scheduleState]}
                              </span>
                              ...
                              <small className="calendar-event-teacher">
                                {session.teacherName}
                              </small>
                            </>
                          )}
