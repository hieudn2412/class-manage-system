...
export type SalaryEditor =
  { kind: "adjustment"; item?: SalaryAdjustment } | { kind: "payment"; item?: SalaryPayment };

interface Props {
  editor: SalaryEditor | null;
  teacherId: string;
  month: string;
...
export const SalaryTransactionModal = ({
  editor,
  teacherId,
  month,
  teachers,
  ...
  const [selectedTeacher, setSelectedTeacher] = useState(
    adjustment?.teacherId ?? payment?.teacherId ?? teacherId,
  );
  const [salaryMonth, setSalaryMonth] = useState(
    adjustment?.salaryMonth ?? payment?.salaryMonth ?? month,
  );
  ...
  const available =
    outstanding +
    (payment?.teacherId === selectedTeacher && payment.salaryMonth === salaryMonth
      ? payment.amount
      : 0);
  const isOverpayment =
    editor?.kind === "payment" &&
  ...
  const title = useMemo(
    () =>
      editor?.kind === "adjustment"
        ? `${isEdit ? "Sửa" : "Thêm"} khoản cộng / trừ`
        : `${isEdit ? "Sửa" : "Ghi"} thanh toán lương`,
    [editor?.kind, isEdit],
  );
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (editor.kind === "adjustment") {
        ...
        return adjustment
          ? salaryRepository.updateAdjustment(tenant.slug, adjustment.id, {
              ...input,
              editReason,
              version: adjustment.version,
            })
          : salaryRepository.createAdjustment(tenant.slug, input);
      }
      const input = {
      ...
      return payment
        ? salaryRepository.updatePayment(tenant.slug, payment.id, {
            ...input,
            editReason,
            version: payment.version,
          })
        : salaryRepository.createPayment(tenant.slug, input);
    },
    onSuccess: () => {
      onSaved();
      onClose();
  ...
  const invalid =
    ...
    (editor?.kind === "adjustment"
      ...
      : numericAmount < 0 ||
        ...
        (isOverpayment && (!confirmOverpayment || !overpaymentReason.trim()))) ||
    (isEdit && !editReason.trim());
  return (
    <Modal
      open={Boolean(editor)}
      title={title}
      onClose={onClose}
      confirmLabel="Lưu vào sổ lương"
    ...
    >
      <div className="salary-form-grid">
        ...
        <Input
          ...
          hint={editor?.kind === "adjustment" ? "Dùng số âm cho khoản trừ." : undefined}
        />
        {editor?.kind === "adjustment" ? (
          <Textarea
            ...
            <Input
              ...
              hint={
                method === "BANK_TRANSFER"
                  ? "Vui lòng nhập mã giao dịch hoặc nội dung chuyển khoản."
                  : "Không bắt buộc khi trả tiền mặt."
              }
            />
            {isOverpayment ? (
              <div className="overpayment-confirm">
                <AlertTriangle size={19} />
                <div>
                  <strong>
                    Khoản trả vượt{" "}
                    {formatCurrency(Math.max(0, numericAmount - Math.max(available, 0)))}
                  </strong>
                  <p>
                    Số tiền đã trả có thể lớn hơn số tiền cần thanh toán và sẽ không tự bù sang kỳ
                    sau.
                  </p>
                  <label>
                    <input
                      ...
                      onChange={(event) => setConfirmOverpayment(event.target.checked)}
                    />{" "}
                    Tôi đã kiểm tra và vẫn muốn ghi khoản trả vượt
                  </label>
                  <Textarea
                    label="Lý do trả vượt"
        ...
        {isEdit ? (
          <Textarea
            label="Lý do sửa giao dịch"
            value={editReason}
            onChange={(event) => setEditReason(event.target.value)}
            hint="Vui lòng nhập lý do. Thông tin trước và sau khi sửa sẽ được lưu trong lịch sử."
          />
        ) : null}
      </div>
      {error ? (
