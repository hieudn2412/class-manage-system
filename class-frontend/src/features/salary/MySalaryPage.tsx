...
export const MySalaryPage = () => {
  ...
  return <section className="salary-page my-salary-page"><header className="salary-header"><div><p className="eyebrow">WF-22 · MY SALARY</p><h1>Giờ dạy & lương của tôi</h1><p>Mỗi khoản phát sinh đều truy được thời lượng và đơn giá lớp tại ngày dạy.</p></div><label className="month-control"><span>Kỳ lương</span><input type="month" value={month} onChange={(event) => setMonth(event.target.value)} /></label></header>
    <div className="my-salary-hero"><div className="my-salary-seal"><Wallet size={26} /><span>{formatMonth(month)}</span></div><div><small>Còn phải trả</small><strong className={detail.metrics.outstanding < 0 ? "amount-negative" : ""}>{formatCurrency(detail.metrics.outstanding)}</strong><Badge tone={tone}>{detail.metrics.status === "OWED" ? "Còn phải trả" : detail.metrics.status === "OVERPAID" ? "Trả thừa" : "Đã cân bằng"}</Badge></div><dl><div><dt>Tổng giờ</dt><dd>{(detail.metrics.totalMinutes / 60).toLocaleString("vi-VN", { maximumFractionDigits: 1 })}</dd></div><div><dt>Tiền dạy</dt><dd>{formatCurrency(detail.metrics.accrued)}</dd></div><div><dt>Cộng / trừ</dt><dd>{formatCurrency(detail.metrics.adjustments)}</dd></div><div><dt>Đã trả</dt><dd>{formatCurrency(detail.metrics.paid)}</dd></div></dl></div>
    <div className="salary-live-notice"><CircleAlert size={18} /><p><strong>Số liệu có thể thay đổi.</strong> Nếu thời lượng, giáo viên thực tế hoặc đơn giá hiệu lực của buổi cũ được sửa, hệ thống sẽ tính lại và lưu revision.</p></div>
