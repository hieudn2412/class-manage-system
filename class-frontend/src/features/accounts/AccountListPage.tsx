...
const profileLabels = { STAFF: "Nhân sự", TEACHER: "Giáo viên", STUDENT: "Học sinh" } as const;
...
export const AccountListPage=()=>{
  ...
  return <section className="management-page"><header className="management-header"><div><p className="eyebrow">QUẢN LÝ TÀI KHOẢN</p><h1>Người dùng trung tâm</h1><p>Quản lý tài khoản, vai trò và hồ sơ giáo viên, học sinh của trung tâm.</p></div><Link className="button" to={`${base}/new`}><Plus size={17}/>Tạo tài khoản</Link></header>
    <div className="filter-bar"><label className="search-box"><Search size={17}/><input value={search} onChange={e=>{setSearch(e.target.value);setPage(1)}} placeholder="Tên, tên đăng nhập, email hoặc mã…"/></label><select value={profile} onChange={e=>setProfile(e.target.value)}><option value="">Mọi loại hồ sơ</option><option value="STAFF">Nhân sự</option><option value="TEACHER">Giáo viên</option><option value="STUDENT">Học sinh</option></select><select value={status} onChange={e=>setStatus(e.target.value)}><option value="">Mọi trạng thái</option><option value="ACTIVE">Hoạt động</option><option value="LOCKED">Đã khóa</option></select></div>
    {query.isError?<StatePanel kind="error" title="Không tải được tài khoản" description="Kiểm tra kết nối rồi thử lại." actionLabel="Thử lại" onAction={()=>void query.refetch()}/>:query.data?.items.length===0?<StatePanel kind="empty" title="Không có tài khoản phù hợp" description="Thay đổi bộ lọc hoặc tạo tài khoản mới."/>:<><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Tài khoản</th><th>Hồ sơ</th><th>Vai trò</th><th>Trạng thái</th><th>Thao tác</th></tr></thead><tbody>{query.data?.items.map(a=><tr key={a.id}><td><Link to={`${base}/${a.id}`}><strong>{a.displayName}</strong></Link><small>@{a.username} {a.email&&`· ${a.email}`}</small></td><td><strong>{a.code??profileLabels[a.profileType]}</strong><small>{profileLabels[a.profileType]}</small></td><td><div className="badge-row">{a.roles.map(r=><Badge key={r}>{roleLabels[r]}</Badge>)}</div></td><td><Badge tone={a.status==="ACTIVE"?"success":"danger"}>{a.status==="ACTIVE"?"Hoạt động":"Đã khóa"}</Badge></td><td><div className="row-actions"><Button variant="secondary" onClick={()=>setTarget({account:a,kind:"status"})}><LockKeyhole size={15}/>{a.status==="ACTIVE"?"Khóa tài khoản":"Mở lại tài khoản"}</Button><Button variant="ghost" onClick={()=>setTarget({account:a,kind:"reset"})}><RotateCcw size={15}/>Đặt lại mật khẩu</Button></div></td></tr>)}</tbody></table></div>{query.data&&<Pagination page={query.data.page} totalPages={query.data.totalPages} totalItems={query.data.totalItems} itemLabel="tài khoản" onPageChange={setPage}/>}</>}
    <Modal open={Boolean(target)} title={target?.kind==="reset"?"Đặt lại mật khẩu":"Đổi trạng thái tài khoản"} onClose={()=>setTarget(null)} confirmLabel="Xác nhận" confirmDisabled={!reason.trim()} confirmLoading={mutation.isPending} onConfirm={()=>mutation.mutate()}><p>Vui lòng nhập lý do. Thay đổi sẽ được lưu vào lịch sử và các phiên đăng nhập cũ sẽ tự kết thúc.</p><Input label="Lý do" value={reason} onChange={e=>setReason(e.target.value)}/></Modal>
