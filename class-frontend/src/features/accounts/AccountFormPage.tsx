...
export const AccountFormPage=()=>{
  ...
  return <section className="management-page account-form-page"><Link className="back-link" to={`/t/${tenantSlug}/app/accounts`}><ArrowLeft size={16}/>Danh sách tài khoản</Link><header className="management-header"><div><p className="eyebrow">{editing?"CHỈNH SỬA":"TẠO MỚI"}</p><h1>{editing?detail.data?.displayName:"Tạo tài khoản"}</h1><p>Sau khi tạo, tên đăng nhập, loại hồ sơ và mã giáo viên hoặc học sinh sẽ không thể thay đổi.</p></div></header>
    <form className="surface-form" onSubmit={e=>{e.preventDefault();mutation.mutate()}}>{error&&<div className="form-alert full">{error}</div>}<div className="form-grid"><Select label="Loại hồ sơ" value={profile} disabled={editing} onChange={e=>changeProfile(e.target.value as ProfileType)}><option value="STAFF" disabled={!isAdmin}>Nhân sự</option><option value="TEACHER">Giáo viên</option><option value="STUDENT">Học sinh</option></Select><Input label="Tên đăng nhập" value={username} disabled={editing} onChange={e=>setUsername(e.target.value)}/><Input label="Tên hiển thị" value={name} onChange={e=>setName(e.target.value)}/><Input label="Email" type="email" value={email} onChange={e=>setEmail(e.target.value)}/>{profile==="STUDENT"&&<><Input label="Tên phụ huynh" value={parentName} onChange={e=>setParentName(e.target.value)}/><Input label="Số điện thoại phụ huynh" value={parentPhone} onChange={e=>setParentPhone(e.target.value)}/></>}</div><fieldset className="role-fieldset"><legend>Vai trò</legend><div className="role-picker">{roleOptions.map(role=><label key={role}><input type="checkbox" checked={roles.includes(role)} disabled={role==="TEACHER"||role==="STUDENT"} onChange={e=>setRoles(e.target.checked?[...roles,role]:roles.filter(r=>r!==role))}/><span>{roleLabels[role]}</span></label>)}</div></fieldset><div className="form-actions"><Button type="submit" loading={mutation.isPending} disabled={!username||!name||roles.length===0}><Save size={17}/>Lưu tài khoản</Button></div></form>
  </section>;
};
