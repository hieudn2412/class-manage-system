...
| DEC-054 | FL-10 dùng snapshot người nhận khi publish BTVN; học sinh vào muộn không tự nhận, vào lại lớp được thấy bài cũ nếu từng thuộc snapshot, và chỉ bỏ người nhận chưa từng nộp. | Tỷ lệ nộp/chữa ổn định, không làm thay đổi nghĩa vụ học tập lịch sử khi enrollment thay đổi. | Chốt |
| DEC-055 | Tài liệu/BTVN dùng private filesystem một backend replica, upload staging token 24 giờ, ClamAV đồng bộ, whitelist PDF/ảnh/HEIC/audio/Office/TXT và quota mặc định 50GB/tenant. | Giữ file ngoài web root, có thể vận hành ngay bằng Docker named volume; scale ngang sẽ là hạng mục lưu trữ sau. | Chốt |
| DEC-056 | File editable của tài liệu, đề bài và phản hồi bị xóa vật lý khi thay/gỡ; file bài nộp học sinh là bất biến và không bị xóa. Audit giữ metadata/checksum. | Cân bằng quyền xóa vận hành với tính toàn vẹn bài nộp và lịch sử chữa bài. | Chốt |
| DEC-057 | Kế toán có quyền đọc đầy đủ BTVN, tài liệu, bài nộp, review và file phản hồi nhưng không được ghi; giáo viên chỉ quản lý theo GV chính/phân công hoặc giáo viên thực tế của buổi đã hoàn tất. | Bảo đảm kiểm toán nghiệp vụ/tài chính có dữ liệu đầy đủ mà không mở quyền sửa nội dung học tập. | Chốt |
| DEC-058 | Mỗi tenant có đúng một Gmail sender hoạt động do Admin tenant kết nối bằng OAuth; cùng Gmail có thể dùng ở nhiều tenant; disconnect chỉ xóa token cục bộ, không gọi Google revoke. Email tenant giữ tối đa 72 giờ trong outbox và không dùng SMTP global. | SaaS cần mỗi trung tâm gửi thông báo bằng hộp thư riêng, không chia sẻ Gmail App Password toàn hệ thống; tránh revoke nhầm Gmail đang dùng cho tenant khác. | Chốt |
